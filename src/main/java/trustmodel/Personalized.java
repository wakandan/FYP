...
public class Personalized {
	 ...
	 */
	double			lamda;
	double			gamma;			// this is the confidence measure
									// when determining the weight of
									// private reputation
	double			epsilon;		// this is the acceptable error for
									// determining the weight of private
									// reputation and the threshold for
									// determining the consistency of
									// advisor's ratings with the
									// majority ratings
	double			inconsistency;
	double			forgetting;
	int				timeWindow;	/* Do a fixed time window for now */
	...
	public double personalized(String buyerName, String advisorName, ArrayList<String> allBuyers) {
	...
	private double getPrivateRep(String buyerName, String advisorName) {
		...
		try {
			...
			st = ratingManager.getDb().prepare(
